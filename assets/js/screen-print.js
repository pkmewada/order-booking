$(document).ready(async function () {
    ...
    const WORK_STORAGE_KEY = "addWork_screen_print";
    const WORK_NEXT_ID_KEY = "addWork_screen_print_nextId";
    const WORK_HISTORY_KEY = "addWork_screen_print_history";

    const WORK_TYPE = "Screen Print";
    ...
    async function productionAction(action, modal) {
        try {
            ...
            loadData(); renderAvailableTable(); renderAssignedTable();
    ...
    function loadData() {
        const view = Production.views("addWork_screen_print");
        pool = view.pool;
        workData = view.works;
        nextId = Math.max(0, ...workData.map(w => Number(w.id) || 0)) + 1;
    }
    ...
    function isUntouched(item) {
        return (item.completedQty || 0) === 0 && (item.damageQty || 0) === 0 && (item.passedQty || 0) === 0;
    }
    ...
    function getPoolTotal(p) { return Number(p.stageBalances?.[Production.managers["addWork_screen_print"]]?.inputQty) || 0; }
    function getPoolAssigned(p) {
        return workData.filter(w => String(w.poolId) === String(p.id)).reduce((n,w) => n + w.assignedQty, 0);
    }
    function getPoolRemaining(p) { return Production.calculateAvailableQty(p, workData, Production.managers["addWork_screen_print"]); }
    ...
    function renderAssignedTable() {
        ...
        Object.keys(grouped).forEach(batchId => {
            ...
            items.forEach((item, idx) => {
                ...
                const progress = item.completedQty || 0, passed = item.passedQty || 0;
                const eff = Production.calculateProductionMath(item).effectiveQty;
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                ...
                                ${canPass ? `<button class="btn btn-sm pass-row-btn pass-row-action-btn" data-id="${item.id}" ${canPass ? "" : "disabled"}><i class="bx bx-right-arrow-alt"></i></button>` : ""}
    ...
    $("#saveAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("addWork_screen_print", collectAssignments(".assignment-row", false)), "#assignModal");
    });
    ...
    $("#saveBulkAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("addWork_screen_print", collectAssignments(".bulk-assignment-row", true)), "#bulkAssignModal");
    });
    ...
    function refreshProgressNumbers(item) {
        ...
        const passed = item.passedQty || 0, qty = item.assignedQty || 0;
        const eff = Production.calculateProductionMath(item).effectiveQty;
        const remaining = Production.calculateProductionMath(item).uncompletedQty;
    ...
    $(document).on("input change", "#progressQty, #progressTypeSelect", function () {
        ...
        const eff = Production.calculateProductionMath({ ...item, damageQty: previewDamage }).effectiveQty;
    ...
    $("#updateProgressBtn").on("click", async function () {
        ...
        if (type === "recover") { await productionAction(() => Production.recoverRepair("addWork_screen_print", currentEditingId, qty), "#progressModal"); return; }
        ...
        await productionAction(() => Production.edit("addWork_screen_print", [{ id: currentEditingId, [fields[type]]: qty }]), "#progressModal");
    });

    /* ============================================================
       PASS TO NEXT STAGE
       ============================================================ */
    function pushToNextStage(item) {
        return Production.pass("addWork_screen_print", [item.id]);
    }
    ...
    $(document).on("click", ".pass-row-action-btn", async function () {
        ...
        const passed = await productionAction(() => Production.pass("addWork_screen_print", [id]));
    ...
    $(document).on("click", ".stop-row-action-btn", async function () {
        const id = Number($(this).data("id"));
        await productionAction(() => Production.stop("addWork_screen_print", id));
    });
    ...
    function buildListAllHtml() {
        ...
        batchIds.forEach(batchId => {
            ...
            items.forEach((item, idx) => {
                ...
                const remaining = (item.assignedQty || 0) - (item.completedQty || 0);
                tableRows += `
                    <tr>
                        <td>${isFirst ? serial : ""}</td>
    ...
    /* ============================================================
       REFRESH
       ============================================================ */
    $("#refreshScreenBtn").on("click", function () {
        loadData();
    ...
    installProductionControls("addWork_screen_print", () => { loadData(); renderAvailableTable(); renderAssignedTable(); });
