$(document).ready(async function () {
    ...
    function loadData() {
        const view = Production.views("ironingData");
        pool = view.pool;
        workData = view.works;
    ...
    function getPoolTotal(p) { return Number(p.stageBalances?.[Production.managers["ironingData"]]?.inputQty) || 0; }
    function getPoolAssigned(p) {
        return workData.filter(w => String(w.poolId) === String(p.id)).reduce((n,w) => n + w.assignedQty, 0);
    }
    function getPoolRemaining(p) { return Production.calculateAvailableQty(p, workData, Production.managers["ironingData"]); }
    ...
    $("#saveAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("ironingData", collectAssignments(".assignment-row", false)), "#assignModal");
    });
    ...
    $("#saveBulkAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("ironingData", collectAssignments(".bulk-assignment-row", true)), "#bulkAssignModal");
    });
    ...
    function renderProgressHistory(ironId) {
        const $list = $("#progressHistoryList");
        if (!$list.length) return;
        $list.empty();

        const history = getHistoryFor(ironId);
        if (!history.length) {
    ...
    $("#updateProgressBtn").on("click", async function () {
        ...
        if (type === "recover") { await productionAction(() => Production.recoverRepair("ironingData", currentEditingId, qty), "#progressModal"); return; }
        ...
        await productionAction(() => Production.edit("ironingData", [{ id: currentEditingId, [fields[type]]: qty }]), "#progressModal");
    });
    ...
    function pushToNextStage(item) {
        return Production.pass("ironingData", [item.id]);
    }
    ...
    $(document).on("click", ".pass-row-action-btn", async function () {
        ...
        const passed = await productionAction(() => Production.pass("ironingData", [id]));
    ...
    $(document).on("click", ".stop-row-action-btn", async function () {
        const id = Number($(this).data("id"));
        await productionAction(() => Production.stop("ironingData", id));
    });
    ...
    function buildBatchHistoryHtml(batchId) {
        ...
        subBatchKeys.forEach((subBatch, idx) => {
            ...
            const allEvents = history.filter(h => itemIds.includes(Number(h.ironId)))
    ...
    installProductionControls("ironingData", () => { loadData(); renderAvailableTable(); renderAssignedTable(); });
