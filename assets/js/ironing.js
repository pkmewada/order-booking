$(document).ready(function () {
    ...
    $("#saveBulkAssignBtn").on("click", function () {
        ...
        assignments.forEach(a => {
            ...
            pushHistory({ ironId: newId, batchId: poolItem.batchId, subBatch: a.subBatch, action: `Bulk assigned ${a.qty} pcs to ${a.worker}`, by: "Manager" });
            addedCount++;
        });
    ...
    function pushToNextStage(item, qty) {
        ...
        if (nextStage.type === "packing") {
            // Merge into the existing packing record for this piece so partial passes add up
            // (existing packing state is kept, only the quantity grows).
            const packingPool = readStorage("packingPool");
            const pIdx = packingPool.findIndex(p => String(p.batchId) === String(item.batchId) && Number(p.pieceNumber) === Number(item.pieceNumber));
            if (pIdx !== -1) {
                packingPool[pIdx] = { ...packingPool[pIdx], quantity: (Number(packingPool[pIdx].quantity) || 0) + qty, updatedAt: now };
            } else {
                packingPool.push({
