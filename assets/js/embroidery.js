$(document).ready(async function () {
    ...
    const WORK_STORAGE_KEY = "addWork_embroidery";
    const WORK_NEXT_ID_KEY = "addWork_embroidery_nextId";
    ...
    function loadData() {
        const view = Production.views("addWork_embroidery");
        pool = view.pool;
        workData = view.works;
    ...
    function getPoolTotal(p) { return Number(p.stageBalances?.[Production.managers["addWork_embroidery"]]?.inputQty) || 0; }
    function getPoolAssigned(p) {
        return workData.filter(w => String(w.poolId) === String(p.id)).reduce((n,w) => n + w.assignedQty, 0);
    }
    function getPoolRemaining(p) { return Production.calculateAvailableQty(p, workData, Production.managers["addWork_embroidery"]); }
    ...
    $("#saveAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("addWork_embroidery", collectAssignments(".assignment-row", false)), "#assignModal");
    });
    ...
    $("#saveBulkAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("addWork_embroidery", collectAssignments(".bulk-assignment-row", true)), "#bulkAssignModal");
    });
    ...
    $("#updateProgressBtn").on("click", async function () {
        ...
        if (type === "recover") { await productionAction(() => Production.recoverRepair("addWork_embroidery", currentEditingId, qty), "#progressModal"); return; }
        ...
        await productionAction(() => Production.edit("addWork_embroidery", [{ id: currentEditingId, [fields[type]]: qty }]), "#progressModal");
    });
    ...
    function pushToNextStage(item) {
        return Production.pass("addWork_embroidery", [item.id]);
    }
    ...
    $(document).on("click", ".pass-row-action-btn", async function () {
        ...
        const passed = await productionAction(() => Production.pass("addWork_embroidery", [id]));
    ...
    $(document).on("click", ".stop-row-action-btn", async function () {
        const id = Number($(this).data("id"));
        await productionAction(() => Production.stop("addWork_embroidery", id));
    });
    ...
    /* ============================================================
       REFRESH
       ============================================================ */
    $("#refreshEmbroideryBtn").on("click", function () {
        loadData();
    ...
    installProductionControls("addWork_embroidery", () => { loadData(); renderAvailableTable(); renderAssignedTable(); });
