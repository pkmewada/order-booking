$(document).ready(async function () {
    ...
    const SUB_BATCH_SUFFIX = "C";   // ✅ -C1 (matches Cutting Manager / Embroidery)
    ...
    function loadData() {
        const view = Production.views("addWork_digital_print");
        pool = view.pool;
        workData = view.works;
    ...
    function getPoolTotal(p) { return Number(p.stageBalances?.[Production.managers["addWork_digital_print"]]?.inputQty) || 0; }
    function getPoolAssigned(p) {
        return workData.filter(w => String(w.poolId) === String(p.id)).reduce((n,w) => n + w.assignedQty, 0);
    }
    function getPoolRemaining(p) { return Production.calculateAvailableQty(p, workData, Production.managers["addWork_digital_print"]); }
    ...
    $("#saveAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("addWork_digital_print", collectAssignments(".assignment-row", false)), "#assignModal");
    });
    ...
    $("#saveBulkAssignBtn").on("click", async function () {
        await productionAction(() => Production.assign("addWork_digital_print", collectAssignments(".bulk-assignment-row", true)), "#bulkAssignModal");
    });
    ...
    $("#updateProgressBtn").on("click", async function () {
        ...
        if (type === "recover") { await productionAction(() => Production.recoverRepair("addWork_digital_print", currentEditingId, qty), "#progressModal"); return; }
        ...
        await productionAction(() => Production.edit("addWork_digital_print", [{ id: currentEditingId, [fields[type]]: qty }]), "#progressModal");
    });
    ...
    function pushToNextStage(item) {
        return Production.pass("addWork_digital_print", [item.id]);
    }
    ...
    $(document).on("click", ".pass-row-action-btn", async function () {
        ...
        const passed = await productionAction(() => Production.pass("addWork_digital_print", [id]));
    ...
    $(document).on("click", ".stop-row-action-btn", async function () {
        const id = Number($(this).data("id"));
        await productionAction(() => Production.stop("addWork_digital_print", id));
    });
    ...
    installProductionControls("addWork_digital_print", () => { loadData(); renderAvailableTable(); renderAssignedTable(); });
